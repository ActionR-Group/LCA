...
if __name__ == "__main__":
    ...
    parser.add_argument('--motion-curl-dir', default=None)
    parser.add_argument('--bone-motion-curl-dir', default=None)
    ...
    if arg.joint_curl_dir is not None:
        with open(os.path.join(arg.joint_curl_dir, 'epoch1_test_score.pkl'), 'rb') as r5:  # joint_curl
            r5 = list(pickle.load(r5).items())
    if arg.bone_curl_dir is not None:
        with open(os.path.join(arg.bone_curl_dir, 'epoch1_test_score.pkl'), 'rb') as r6:  # bone_sph
            r6 = list(pickle.load(r6).items())
    if arg.motion_curl_dir is not None:
        with open(os.path.join(arg.motion_curl_dir, 'epoch1_test_score.pkl'), 'rb') as r7:  # motion_sph
            r7 = list(pickle.load(r7).items())

    if arg.bone_motion_curl_dir is not None:
        with open(os.path.join(arg.bone_motion_curl_dir, 'epoch1_test_score.pkl'), 'rb') as r8:  # motion_bone_sph
    ...
    elif arg.joint_motion_dir is not None and arg.bone_motion_dir is not None and arg.joint_curl_dir is not None and arg.bone_curl_dir is not None and arg.motion_curl_dir is not None and arg.bone_motion_curl_dir is not None:
        arg.alpha = [0.6, 0.6, 0.4, 0.4,
                     0.6, 0.6, 0.4, 0.4]
        for i in tqdm(range(len(label))):
            l = label[i]
            ...
            _, r77 = r7[i]
            _, r88 = r8[i]
            r = r11 * arg.alpha[0] + r22 * arg.alpha[1] + r33 * arg.alpha[2] + r44 * arg.alpha[3] + r55 * arg.alpha[4] + r66 * arg.alpha[5] + r77 * arg.alpha[6] + r88 * arg.alpha[7]
            rank_5 = r.argsort()[-5:]
            right_num_5 += int(int(l) in rank_5)
            r = np.argmax(r)
            right_num += int(r == int(l))
