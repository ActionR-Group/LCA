...
class Feeder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'curl' in self.data_path:
            data_curl = np.zeros_like(data)
            data_numpy = np.transpose(data_curl, (1, 2, 0, 3))
            curl, div = calcu_curl(data_numpy)
            data = curl * np.transpose(data_numpy, (2, 0, 1, 3))
...
def calcu_curl(data_numpy):
    T, V, C = data_numpy.shape  # CTVM
    u = np.zeros([T, V, 3])
    omega = np.zeros([T, V, 3])
    alpha = np.zeros([T, V, 3])
    angular_v = np.zeros([T, V, 3])
    angular_a = np.zeros([T, V, 3])

    parents = np.array(
        [0, 1, 2, 3, 3, 5, 6, 7, 3, 9, 10, 11, 1, 13, 14, 15, 1, 17, 18, 19])
    for j in range(T):
        for i in range(1, V):
